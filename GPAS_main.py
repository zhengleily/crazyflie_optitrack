    ...
    for i in range(int(args['max_episodes'])):
        for el in range(1):
            ...
            for j in range(int(args['max_episode_len'])):
                ...
                std = np.zeros([9,1])
                u_bar_, v_t_pos, v_t_angle = cbf.control_barrier(agent, np.squeeze(s), action_RL, f, g, x, std, j,True)
            ...
            if el <= 1:
                ...
                np.save('GPAS.npy',np.vstack(v_pos))
                obs = np.vstack(obs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max-episode-len', help='max length of 1 episode', default=1500)  # 1000 default
